...
namespace DictionaryBit.TelegramInteraction.Operations.Command.WordCUD.Update.States
{
    public class EditWordState : IState
    {
        ...
        public EditWordState(RepositoryManager repositoryManager, WordInteraction wordInteraction, TelegramBotClient botClient, ISession session, User user)
        {
            _session = session;
            _user = user;
        ...
        public async Task ExecuteAsync()
        {
            ...
            var wordId = _session.Get<Guid>(SessionKeyNames.EditingWordId);
            if (dictionaryId == default)
                throw new InvalidOperationException();
            var word = _repositoryManager.WordRepository.GetById(wordId);
            ...
            switch (result)
            {
                case Enums.SaveWordResult.Ok:
                    ...
                    await _botClient.SendTextMessageAsync(_user.ChatId, $"Слово изменено");
                    break;
